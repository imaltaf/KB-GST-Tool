...
function copyFormattedContent() {
  ...
  // Format RegDate and ExpiryDate
  const formattedRegDate = RegDate ? `${RegDate.split("-").reverse().join("/")}` : "NA";
  const formattedExpiryDate = ExpiryDate ? `${ExpiryDate.split("-").reverse().join("/")}` : "NA";
